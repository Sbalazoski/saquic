...
// Note: Vite handles CSS via import in main.jsx
// Using inline styles for critical loading to ensure no FOUC
const styles = {
  ...
  button: { width: '100%', maxWidth: '280px', backgroundColor: '#A0522D', color: 'white', fontWeight: '500', padding: '14px 20px', borderRadius: '8px', border: 'none', cursor: 'pointer', outline: 'none' },
  buttonFocus: { boxShadow: '0 0 0 3px rgba(160, 82, 45, 0.5)' },
}
...
function LandingPage() {
  ...
  function handleStart() {
    try {
      ...
      setError('Please enable cookies/localStorage to use this app')
    }
  }
  ...
  const buttonStyle = {
    ...styles.button,
    ...(buttonFocused ? styles.buttonFocus : {}),
  }
    ...
    <div style={styles.container}>
      <div style={styles.content}>
        ...
        <button 
          ...
          aria-label="Get started with SAQUIC app"
          role="button"
          tabIndex="0"
        >
          Get Started
        </button>
        {error && <p style={{...styles.subtitle, color: '#c00', marginTop: '16px'}} role="alert">{error}</p>}
      </div>
    </div>
  )
}
